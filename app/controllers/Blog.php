...
class Blog extends Controller {
    ...
    public function index() {
        ...
        /* Blog category */
            ...
            $total_rows = database()->query("SELECT COUNT(*) AS `total` FROM `blog_posts` WHERE `blog_posts_category_id` = {$blog_posts_category->blog_posts_category_id} AND (`language` = '{$language}' OR `language` IS NULL) AND `is_published` = 1 {$filters->get_sql_where()}")->fetch_object()->total ?? 0;
        ...
        /* Blog index */
            ...
            /* Prepare the paginator */
            $total_rows = database()->query("SELECT COUNT(*) AS `total` FROM `blog_posts` WHERE (`language` = '{$language}' OR `language` IS NULL) AND `is_published` = 1 {$filters->get_sql_where()}")->fetch_object()->total ?? 0;
